...
func _refresh(message: String = "") -> void:
	...
	info.text = "[b]%s[/b]  %s\nPopulation: %d  Happiness: %.0f  Corruption: %.1f\nIndustry: %s\nMayor: %s (%s)\nHouse: %s, influence %s\nCulture: %s\nStorage: %s\nNeeds: %s\nShortages: %s" % [settlement.name, settlement.level_name(), settlement.population, settlement.happiness, settlement.corruption, industry.get("name", settlement.industry_id), mayor.get("name", "None"), mayor.get("trait", "None"), house.get("name", "None"), str(house.get("influence", 0)), culture.get("name", "None"), _dict_text(settlement.storage), _dict_text(settlement.needs), _dict_text(settlement.shortages)]
