...
const HearthTileDataScript = preload("res://scripts/models/TileData.gd")
...
func load_starting_map() -> void:
	...
	for y in range(height):
		for x in range(width):
			tiles.append(HearthTileDataScript.new({"x": x, "y": y, "terrain": rows[y][x]}))
...
func apply_save(data: Dictionary) -> void:
	width = int(data.get("width", 5)); height = int(data.get("height", 4))
	tiles.clear(); settlements.clear(); routes.clear()
	for tile_data in data.get("tiles", []): tiles.append(HearthTileDataScript.new(tile_data))
